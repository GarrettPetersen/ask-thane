...
-- Source conversations captured from providers (Slack/Teams/etc.).
...
-- Cached membership graph per source conversation.
...
-- ACL links from resources to source conversations.
...
-- Ingest idempotency + provenance for provider events.
...
-- Existing-table links for provenance and ACL projection.
ALTER TABLE tasks
  ADD COLUMN primary_conversation_source_id TEXT REFERENCES conversation_sources(id) ON DELETE SET NULL;

ALTER TABLE task_events
  ADD COLUMN conversation_source_id TEXT REFERENCES conversation_sources(id) ON DELETE SET NULL;

ALTER TABLE reminders
  ADD COLUMN source_task_event_id TEXT REFERENCES task_events(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_tasks_org_primary_conversation
  ON tasks(organization_id, primary_conversation_source_id);
CREATE INDEX IF NOT EXISTS idx_task_events_org_conversation
  ON task_events(organization_id, conversation_source_id);
CREATE INDEX IF NOT EXISTS idx_reminders_org_source_task_event
  ON reminders(organization_id, source_task_event_id);
