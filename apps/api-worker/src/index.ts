...
import type { PingLocation } from "@ask-thane/domain";
...
interface ThaneCliReadStatePayload {
  workspaceId?: unknown;
  channelId?: unknown;
  channelName?: unknown;
}
...
interface ThaneCliNotifyLocationPayload {
  workspaceId?: unknown;
  preferredPingLocation?: unknown;
  location?: unknown;
}

interface ThaneCliWebhookCreatePayload {
  workspaceId?: unknown;
  name?: unknown;
  url?: unknown;
  eventTypes?: unknown;
...
interface ThaneCliWebhookReactionPayload {
  messageId?: unknown;
  emoji?: unknown;
}
...
const THANE_CLI_WEBHOOK_EVENT_TYPES = new Set(["message.created"]);
...
async function uniqueThaneCliHandle(
  ...
  const existingQuery = env.DB
    .prepare("SELECT account_id, email, handle FROM thane_cli_workspace_members WHERE workspace_id = ? AND left_at IS NULL")
...
function coreOrganizationIdForThaneCliWorkspace(workspaceId: string): string {
...
function makeWebhookCredential(prefix: string): string {
...
async function thaneCliAccountState(
  ...
  const workspaceRow = await env.DB
    .prepare("SELECT COUNT(*) AS count FROM thane_cli_workspace_members WHERE email = ? AND left_at IS NULL")
    .bind(email)
    .first<{ count?: number }>();
...
async function ensureThaneCliMember(env: Env, input: {
  ...
  const existingMember = await env.DB
    .prepare("SELECT id, joined_at, left_at FROM thane_cli_workspace_members WHERE workspace_id = ? AND email = ? LIMIT 1")
    .bind(input.workspaceId, input.email)
    .first<{ id?: string; joined_at?: string | null; left_at?: string | null }>();
  await env.DB
    .prepare(
      `INSERT INTO thane_cli_workspace_members (
       ...
       ON CONFLICT(workspace_id, email) DO UPDATE SET
         ...
         joined_at = CASE
           WHEN thane_cli_workspace_members.left_at IS NOT NULL THEN excluded.joined_at
           ELSE thane_cli_workspace_members.joined_at
         END,
         display_name = CASE
         ...
         END,
         handle = CASE
           WHEN thane_cli_workspace_members.left_at IS NOT NULL THEN excluded.handle
         ...
         END,
         role = CASE
           WHEN thane_cli_workspace_members.role = 'owner' AND thane_cli_workspace_members.left_at IS NULL THEN 'owner'
           ELSE excluded.role
         END,
         left_at = NULL,
         updated_at = excluded.updated_at`
  ...
  const row = await env.DB
    .prepare(
      `SELECT id, account_id, email, display_name, handle, role, joined_at
       FROM thane_cli_workspace_members
       WHERE workspace_id = ? AND email = ? AND left_at IS NULL
       LIMIT 1`
    )
    .bind(input.workspaceId, input.email)
  ...
  if ((!existingMember?.id || existingMember.left_at) && row.email !== "thane@askthane.com") {
...
async function countThaneCliWorkspaceOwners(env: Env, workspaceId: string): Promise<number> {
  const row = await env.DB
    .prepare("SELECT COUNT(*) AS count FROM thane_cli_workspace_members WHERE workspace_id = ? AND role = 'owner' AND left_at IS NULL")
    .bind(workspaceId)
    .first<{ count?: number }>();
...
async function ensureThaneCliAppDmChannel(env: Env, input: {
  workspaceId: string;
  appMember: { id: string; handle: string };
  targetMember: { id: string; handle: string };
}): Promise<{ id: string; name: string; kind: "dm"; visibility: "private" }> {
  const appHandle = publicHandleForMember(input.appMember);
  const targetHandle = publicHandleForMember(input.targetMember);
  const name = normalizeChannelName(`dm-${appHandle}-${targetHandle}`) || `dm-${input.appMember.id.slice(0, 8)}-${input.targetMember.id.slice(0, 8)}`;
  const now = nowIso();
  await env.DB
    .prepare(
      `INSERT INTO thane_cli_channels (
         id, workspace_id, name, kind, visibility, topic, created_at, updated_at
       ) VALUES (?, ?, ?, 'dm', 'private', 'App direct messages', ?, ?)
       ON CONFLICT(workspace_id, name) DO UPDATE SET
         updated_at = excluded.updated_at`
    )
  ...
  if (!row?.id || row.kind !== "dm" || row.visibility !== "private" || !row.name) {
    throw new Error("app_dm_channel_upsert_failed");
  }
  for (const memberId of [input.appMember.id, input.targetMember.id]) {
    await env.DB
...
async function softLeaveAskThaneMember(env: Env, workspaceId: string, leftAt: string): Promise<string | null> {
  const row = await env.DB
  ...
  await env.DB
    .prepare("UPDATE thane_cli_channel_members SET left_at = ? WHERE member_id = ? AND left_at IS NULL")
    .bind(leftAt, row.id)
    .run();
  await env.DB
    .prepare("UPDATE thane_cli_workspace_members SET left_at = ?, updated_at = ? WHERE workspace_id = ? AND id = ? AND left_at IS NULL")
...
async function ensureAskThaneWebhookSubscription(env: Env, input: {
  ...
  const existing = await env.DB
    .prepare("SELECT id FROM thane_cli_webhooks WHERE workspace_id = ? AND name = 'Ask Thane' LIMIT 1")
    .bind(input.workspaceId)
    .first<{ id?: string }>();
  const id = existing?.id ?? makeId("twh");
  const token = await deriveAskThaneWebhookToken(env, id);
...
function normalizeWebhookTargetUrl(value: unknown): string | null {
...
function normalizeWebhookEventTypes(value: unknown): string[] {
  const raw = Array.isArray(value) ? value : typeof value === "string" && value.trim() ? [value] : ["message.created"];
  const events = raw
    .map((event) => (typeof event === "string" ? event.trim() : ""))
    .filter((event) => THANE_CLI_WEBHOOK_EVENT_TYPES.has(event));
  return [...new Set(events.length > 0 ? events : ["message.created"])];
}

function parseWebhookEventTypes(value: string | null | undefined): string[] {
  if (!value) {
    return ["message.created"];
  }
  try {
    const parsed = JSON.parse(value) as unknown;
    return normalizeWebhookEventTypes(parsed);
  } catch {
    return ["message.created"];
  }
}

function renderThaneCliWebhook(row: ThaneCliWebhookRow) {
  return {
    id: row.id,
    workspaceId: row.workspace_id,
    name: row.name,
    ...
    ...(row.last_delivered_at ? { lastDeliveredAt: row.last_delivered_at } : {})
  };
}

async function signWebhookBody(secret: string, timestamp: string, body: string): Promise<string> {
  return `v1=${hex(await hmacSha256(`${timestamp}.${body}`, secret))}`;
}

function askThaneWebhookTokenSecret(env: Env): string {
...
async function deriveAskThaneWebhookToken(env: Env, webhookId: string): Promise<string> {
  return `twk_ask_${base64UrlEncode(await hmacSha256(`ask-thane-webhook:${webhookId}`, askThaneWebhookTokenSecret(env)))}`;
}

async function activeThaneCliWebhooksForWorkspace(env: Env, workspaceId: string): Promise<ThaneCliWebhookRow[]> {
  try {
...
async function deliverThaneCliWebhookEvent(env: Env, input: {
  ...
  text: string;
  source: ThaneCliMessageOrigin;
  threadRootId?: string | null;
  createdAt: string;
}): Promise<void> {
  const webhooks = await activeThaneCliWebhooksForWorkspace(env, input.workspaceId);
  if (webhooks.length === 0) {
    return;
  }
  const deliveries = webhooks
    .filter((webhook) => parseWebhookEventTypes(webhook.event_types).includes(input.type))
    .filter((webhook) => webhook.bot_member_id !== input.authorId)
    .map(async (webhook) => {
      const canUseChannel = await canThaneCliMemberUseChannel(env, {
        channelId: input.channelId,
        visibility: input.channelVisibility,
        memberId: webhook.bot_member_id
      });
      if (!canUseChannel) {
        return;
      }
      const deliveryId = makeId("tdlv");
      ...
      const signature = await signWebhookBody(webhook.signing_secret, timestamp, body);
      try {
        const response = await fetch(webhook.target_url, {
          method: "POST",
          headers: {
            "content-type": "application/json",
            "user-agent": "thane-chat-webhooks/1.0",
            "x-thane-delivery-id": deliveryId,
            "x-thane-event": input.type,
            "x-thane-signature": signature,
        ...
        if (response.ok) {
          await env.DB
            .prepare("UPDATE thane_cli_webhooks SET last_delivered_at = ?, updated_at = ? WHERE id = ?")
            .bind(timestamp, timestamp, webhook.id)
            .run();
        } else {
          console.warn("thane_chat_webhook_delivery_failed", {
            workspaceId: input.workspaceId,
...
async function countThaneCliWorkspaceMembers(env: Env, workspaceId: string): Promise<number> {
  const row = await env.DB
    .prepare("SELECT COUNT(*) AS count FROM thane_cli_workspace_members WHERE workspace_id = ? AND left_at IS NULL AND email != 'thane@askthane.com'")
    .bind(workspaceId)
...
async function handleThaneCliProfileUpdate(request: Request, env: Env): Promise<Response> {
  ...
  await env.DB
    .prepare("UPDATE thane_cli_workspace_members SET display_name = ?, updated_at = ? WHERE email = ? AND left_at IS NULL")
...
async function buildThaneCliSyncResponse(request: Request, env: Env): Promise<Response> {
  ...
  const workspaceRows = await env.DB
    .prepare(
      `SELECT w.id, w.workspace_slug, w.workspace_name, w.ascii_art, w.plan_tier, w.created_at, w.updated_at, m.id AS member_id, m.role
       FROM thane_cli_workspace_members m
       JOIN thane_cli_workspaces w ON w.id = m.workspace_id
       WHERE m.email = ? AND m.left_at IS NULL AND w.status = 'active'
       ORDER BY w.workspace_slug`
  ...
  const rawAskThaneIntegration = await askThaneIntegrationForWorkspace(env, activeWorkspace.id);
  const includeAskThaneMember = rawAskThaneIntegration?.enabled === 1;
  ...
  const messageRows = await env.DB
    .prepare(
      `SELECT msg.id, msg.workspace_id, msg.channel_id, msg.author_member_id, msg.text,
              COALESCE(msg.origin, msg.source) AS source, msg.thread_root_id, msg.created_at
  ...
  const readStateRows = currentMember
    ? await env.DB
        .prepare(
          `SELECT workspace_id, channel_id, member_id, last_read_at
           FROM thane_cli_read_states
           WHERE workspace_id = ? AND member_id = ?`
        )
        .bind(activeWorkspace.id, currentMember.id)
        .all<{ workspace_id: string; channel_id: string; member_id: string; last_read_at: string }>()
    : { results: [] as Array<{ workspace_id: string; channel_id: string; member_id: string; last_read_at: string }> };
  const readStates = (readStateRows.results ?? []).map((row) => ({
    workspaceId: row.workspace_id,
    channelId: row.channel_id,
    userId: row.member_id,
    lastReadAt: row.last_read_at
  }));
  const unreadCounts = [...unreadCountsByConversation.values()].filter((count) => count.workspaceId === activeWorkspace.id);
  const workspaceUnreadCounts = workspaces.map((workspace) => {
    const counts = unreadCountsByWorkspace.get(workspace.id) ?? { unreadCount: 0, mentionCount: 0 };
    return {
      workspaceId: workspace.id,
  ...
  const askThaneIntegration = renderAskThaneIntegration(rawAskThaneIntegration);
  const notificationPreference = await readThaneCliNotificationPreferenceForSync(env, {
...
async function handleThaneCliMessageCreate(request: Request, env: Env): Promise<Response> {
  ...
  await env.DB
    ...
    .bind(messageId, workspaceId, channel.id, member.id, text, source, source, threadRootId, createdAt, createdAt)
    .run();
  await broadcastThaneChatEvent(env, {
    type: "message_created",
    workspaceId,
  ...
  await deliverThaneCliWebhookEvent(env, {
    ...
    messageId,
    authorId: member.id,
    authorHandle: publicHandleForMember(member),
    authorDisplayName: publicDisplayNameForMember(member),
    text,
    source,
    threadRootId,
    createdAt
  }).catch((error) => {
    console.warn("thane_chat_webhook_dispatch_failed", {
      workspaceId,
      messageId,
      reason: error instanceof Error ? error.message : String(error)
    });
...
async function handleThaneCliReadStateMark(request: Request, env: Env): Promise<Response> {
  const email = await requireAuthEmail(request, env);
  if (!email) {
    return Response.json({ ok: false, error: "unauthorized" }, { status: 401 });
  }
  const payload = await parseJsonObject<ThaneCliReadStatePayload>(request);
  const workspaceId = typeof payload?.workspaceId === "string" && payload.workspaceId.trim() ? payload.workspaceId.trim() : null;
  const channelId = typeof payload?.channelId === "string" && payload.channelId.trim() ? payload.channelId.trim() : null;
  const channelName = normalizeChannelName(payload?.channelName);
  if (!workspaceId || (!channelId && !channelName)) {
  ...
  const channel = await resolveThaneCliChannel(env, { workspaceId, channelId, channelName });
  if (!channel?.id || !channel.visibility) {
    return Response.json({ ok: false, error: "channel_not_found" }, { status: 404 });
  }
  if (!(await canThaneCliMemberUseChannel(env, { channelId: channel.id, visibility: channel.visibility, memberId: member.id }))) {
    return Response.json({ ok: false, error: "channel_not_found" }, { status: 404 });
  ...
  await env.DB
    .prepare(
      `INSERT INTO thane_cli_read_states (
         id, workspace_id, channel_id, member_id, last_read_at, updated_at
       ) VALUES (?, ?, ?, ?, ?, ?)
       ON CONFLICT(workspace_id, channel_id, member_id) DO UPDATE SET
         last_read_at = excluded.last_read_at,
         updated_at = excluded.updated_at`
    )
    .bind(makeId("tcrs"), workspaceId, channel.id, member.id, now, now)
    .run();
  return Response.json({
    ok: true,
    readState: {
...
async function removeThaneCliWorkspaceMember(env: Env, input: {
  ...
  await env.DB
    .prepare("UPDATE thane_cli_channel_members SET left_at = ? WHERE member_id = ? AND left_at IS NULL")
    .bind(leftAt, input.memberId)
    .run();
  await env.DB
    .prepare("UPDATE thane_cli_workspace_members SET left_at = ?, updated_at = ? WHERE workspace_id = ? AND id = ? AND left_at IS NULL")
    .bind(leftAt, leftAt, input.workspaceId, input.memberId)
    .run();
}
...
async function handleThaneCliWorkspaceMemberRole(request: Request, env: Env): Promise<Response> {
  ...
  await env.DB
    .prepare("UPDATE thane_cli_workspace_members SET role = ?, updated_at = ? WHERE workspace_id = ? AND id = ? AND left_at IS NULL")
...
async function requireWebhookAdmin(request: Request, env: Env, workspaceId: string): Promise<{ email: string; member: NonNullable<Awaited<ReturnType<typeof requireThaneCliWorkspaceMember>>> } | Response> {
  const email = await requireAuthEmail(request, env);
  if (!email) {
    return Response.json({ ok: false, error: "unauthorized" }, { status: 401 });
  }
...
async function handleThaneCliWebhookList(request: Request, env: Env): Promise<Response> {
  const url = new URL(request.url);
  ...
  try {
    const rows = await env.DB
      ...
      .bind(workspaceId)
      .all<ThaneCliWebhookRow>();
    return Response.json({ ok: true, webhooks: (rows.results ?? []).map(renderThaneCliWebhook) });
  } catch (error) {
    if (String(error).toLowerCase().includes("no such table")) {
...
async function handleThaneCliWebhookCreate(request: Request, env: Env): Promise<Response> {
  const payload = await parseJsonObject<ThaneCliWebhookCreatePayload>(request);
  const workspaceId = typeof payload?.workspaceId === "string" && payload.workspaceId.trim() ? payload.workspaceId.trim() : null;
  const name = normalizeWebhookName(payload?.name);
  const targetUrl = normalizeWebhookTargetUrl(payload?.url);
  if (!workspaceId || !name || !targetUrl) {
    return Response.json({ ok: false, error: "workspace_id_name_and_https_url_required" }, { status: 400 });
  }
  const auth = await requireWebhookAdmin(request, env, workspaceId);
  if (auth instanceof Response) {
    return auth;
  ...
  const existing = await env.DB
    .prepare("SELECT id FROM thane_cli_webhooks WHERE workspace_id = ? AND name = ? AND status = 'active' LIMIT 1")
    .bind(workspaceId, name)
    .first<{ id?: string }>();
  if (existing?.id) {
    return Response.json({ ok: false, error: "webhook_name_exists" }, { status: 409 });
  }
  const id = makeId("twh");
  const token = makeWebhookCredential("twk");
  const signingSecret = makeWebhookCredential("whsec");
  const eventTypes = normalizeWebhookEventTypes(payload?.eventTypes);
  const now = nowIso();
  const bot = await ensureThaneCliMember(env, {
    workspaceId,
    email: `webhook+${id}@apps.thane.chat`,
    displayName: name,
    role: "member"
  });
  await env.DB
    ...
    .bind(id, workspaceId, name, targetUrl, JSON.stringify(eventTypes), signingSecret, await sha256Hex(token), bot.id, auth.member.id, now, now)
    .run();
  const row = await env.DB
    .prepare(
      `SELECT webhook.id, webhook.workspace_id, webhook.name, webhook.target_url, webhook.event_types,
  ...
  const postMessageEndpoint = `${origin}/v1/thane-cli/webhooks/messages`;
  const postReactionEndpoint = `${origin}/v1/thane-cli/webhooks/reactions`;
  return Response.json({
    ok: true,
    webhook: row ? renderThaneCliWebhook(row) : {
      id,
      workspaceId,
      name,
      url: targetUrl,
    ...
    signingSecret,
    postMessageEndpoint,
    postReactionEndpoint
  });
}

async function handleThaneCliWebhookDisable(request: Request, env: Env): Promise<Response> {
  const payload = await parseJsonObject<ThaneCliWebhookDisablePayload>(request);
  const workspaceId = typeof payload?.workspaceId === "string" && payload.workspaceId.trim() ? payload.workspaceId.trim() : null;
  const webhookRef = typeof payload?.webhook === "string" && payload.webhook.trim() ? payload.webhook.trim() : null;
  if (!workspaceId || !webhookRef) {
    return Response.json({ ok: false, error: "workspace_id_and_webhook_required" }, { status: 400 });
  }
  const auth = await requireWebhookAdmin(request, env, workspaceId);
  if (auth instanceof Response) {
    return auth;
  ...
  const now = nowIso();
  await removeThaneCliWorkspaceMember(env, { workspaceId, memberId: row.bot_member_id });
  await env.DB
    .prepare("UPDATE thane_cli_webhooks SET status = 'disabled', updated_at = ? WHERE workspace_id = ? AND id = ?")
    .bind(now, workspaceId, row.id)
    .run();
  return Response.json({
    ok: true,
...
async function requireThaneCliWebhookAuth(request: Request, env: Env): Promise<{
  webhook: ThaneCliWebhookRow;
  bot: { id: string; account_id: string; email: string; display_name: string | null; handle: string; role: string };
} | Response> {
  const authHeader = request.headers.get("authorization") ?? "";
  ...
  const bot = await env.DB
    ...
    .bind(webhook.workspace_id, webhook.bot_member_id)
    .first<{ id: string; account_id: string; email: string; display_name: string | null; handle: string; role: string }>();
  if (!bot?.id) {
    return Response.json({ ok: false, error: "webhook_not_active" }, { status: 403 });
  }
  return { webhook, bot };
}

async function handleThaneCliWebhookMessageCreate(request: Request, env: Env): Promise<Response> {
  const auth = await requireThaneCliWebhookAuth(request, env);
  if (auth instanceof Response) {
    return auth;
  }
  const { webhook, bot } = auth;
  const payload = await parseJsonObject<ThaneCliWebhookMessagePayload>(request);
  const text = normalizeChatText(payload?.text);
  const channelId = typeof payload?.channelId === "string" && payload.channelId.trim() ? payload.channelId.trim() : null;
  const channelName = normalizeChannelName(payload?.channelName);
  const dmTarget = typeof payload?.dmTarget === "string" && payload.dmTarget.trim() ? payload.dmTarget.trim() : null;
  if (!text || (!channelId && !channelName && !dmTarget)) {
    return Response.json({ ok: false, error: "channel_or_dm_target_and_text_required" }, { status: 400 });
  }
  const rateLimited = await enforceWorkspaceActionRateLimits(env, {
    action: "webhook_message_create",
  ...
  const targetMember = dmTarget ? await resolveThaneCliWorkspaceMember(env, { workspaceId: webhook.workspace_id, target: dmTarget }) : null;
  if (dmTarget && !targetMember) {
    return Response.json({ ok: false, error: "member_not_found" }, { status: 404 });
  }
  const channel = dmTarget && targetMember
    ? await ensureThaneCliAppDmChannel(env, {
        workspaceId: webhook.workspace_id,
        appMember: bot,
        targetMember
      })
    : await resolveThaneCliChannel(env, { workspaceId: webhook.workspace_id, channelId, channelName });
  if (!channel?.id || !channel.kind || !channel.visibility) {
    return Response.json({ ok: false, error: "channel_not_found" }, { status: 404 });
  }
  if (!(await canThaneCliMemberUseChannel(env, { channelId: channel.id, visibility: channel.visibility, memberId: bot.id }))) {
    return Response.json({ ok: false, error: "channel_not_found" }, { status: 404 });
  }
  const createdAt = nowIso();
  const messageId = makeId("tmsg");
  ...
  await env.DB
    ...
    .bind(messageId, webhook.workspace_id, channel.id, bot.id, text, threadRootId, createdAt, createdAt)
    .run();
  await broadcastThaneChatEvent(env, {
    type: "message_created",
    workspaceId: webhook.workspace_id,
    ...
    console.warn("thane_chat_webhook_message_broadcast_failed", {
      workspaceId: webhook.workspace_id,
      channelId: channel.id,
      reason: error instanceof Error ? error.message : String(error)
    });
  ...
  await deliverThaneCliWebhookEvent(env, {
    ...
    messageId,
    authorId: bot.id,
    authorHandle: publicHandleForMember(bot),
    authorDisplayName: publicDisplayNameForMember(bot),
    text,
    source: "webhook",
    threadRootId,
    createdAt
...
async function handleThaneCliWebhookMessageHistory(request: Request, env: Env): Promise<Response> {
  const auth = await requireThaneCliWebhookAuth(request, env);
  if (auth instanceof Response) {
    return auth;
  }
  ...
  const channelName = normalizeChannelName(url.searchParams.get("channelName"));
  const threadRootId = url.searchParams.get("threadRootId")?.trim() || null;
  const parsedLimit = Number(url.searchParams.get("limit") ?? 50);
  const limit = Number.isFinite(parsedLimit) ? Math.min(Math.max(Math.trunc(parsedLimit), 1), 200) : 50;
  if (!channelId && !channelName) {
    return Response.json({ ok: false, error: "channel_required" }, { status: 400 });
  }
  const rateLimited = await enforceWorkspaceActionRateLimits(env, {
    action: "webhook_message_history",
    workspaceId: webhook.workspace_id,
    memberId: bot.id,
    email: bot.email,
    memberLimit: 600,
    workspaceLimit: 5_000,
    windowSeconds: 60
  });
  if (rateLimited) {
    return rateLimited;
  }
  const channel = await resolveThaneCliChannel(env, { workspaceId: webhook.workspace_id, channelId, channelName });
  if (!channel?.id || !channel.kind || !channel.visibility) {
    return Response.json({ ok: false, error: "channel_not_found" }, { status: 404 });
  }
  if (!(await canThaneCliMemberUseChannel(env, { channelId: channel.id, visibility: channel.visibility, memberId: bot.id }))) {
  ...
  const rows = await env.DB
    .prepare(
      `SELECT msg.id, msg.workspace_id, msg.channel_id, msg.author_member_id, msg.text,
              COALESCE(msg.origin, msg.source) AS source, msg.thread_root_id, msg.created_at,
              member.account_id, member.email, member.display_name, member.handle
       FROM thane_cli_chat_messages msg
    ...
    .bind(webhook.workspace_id, channel.id, threadRootId, threadRootId, threadRootId, limit)
    .all<{
      id: string;
      workspace_id: string;
      channel_id: string;
  ...
  return Response.json({
    ...
    messages: messages.map((message) => ({
      ...
      channelId: message.channel_id,
      authorId: message.author_member_id,
      authorHandle: publicHandleForMember(message),
      authorDisplayName: publicDisplayNameForMember(message),
      text: message.text,
      source: message.source,
      ...(message.thread_root_id ? { threadRootId: message.thread_root_id } : {}),
      createdAt: message.created_at,
...
async function handleThaneCliWebhookReactionCreate(request: Request, env: Env): Promise<Response> {
  const auth = await requireThaneCliWebhookAuth(request, env);
  if (auth instanceof Response) {
    return auth;
  }
  ...
  const message = await env.DB
    ...
    .bind(webhook.workspace_id, messageId)
    .first<{ id: string; channel_id: string; visibility: ThaneCliConversationVisibility }>();
  if (!message?.id) {
    return Response.json({ ok: false, error: "message_not_found" }, { status: 404 });
  }
  if (!(await canThaneCliMemberUseChannel(env, { channelId: message.channel_id, visibility: message.visibility, memberId: bot.id }))) {
    return Response.json({ ok: false, error: "message_not_found" }, { status: 404 });
  }
  const createdAt = nowIso();
  await env.DB
    ...
    .bind(makeId("trxn"), messageId, bot.id, emoji, createdAt)
    .run();
  await broadcastThaneChatEvent(env, {
    type: "reaction_created",
    workspaceId: webhook.workspace_id,
    ...
    console.warn("thane_chat_webhook_reaction_broadcast_failed", {
      workspaceId: webhook.workspace_id,
      channelId: message.channel_id,
      reason: error instanceof Error ? error.message : String(error)
    });
  });
  return Response.json({ ok: true, reaction: { emoji, by: publicHandleForMember(bot), createdAt } });
}
...
async function handleThaneCliAskThaneToggle(request: Request, env: Env, enabled: boolean): Promise<Response> {
  ...
  const now = nowIso();
  const existingIntegration = await askThaneIntegrationForWorkspace(env, workspaceId);
  ...
  await env.DB
    ...
    .bind(
      ...
      enabled ? 1 : 0,
      bot?.id ?? null,
      enabled ? email : existingIntegration?.linked_account_email ?? email,
      existingIntegration?.connected_at ?? now,
      now,
      enabled ? now : existingIntegration?.last_event_at ?? null
    )
    .run();
  const integration = renderAskThaneIntegration(await askThaneIntegrationForWorkspace(env, workspaceId));
  return Response.json({ ok: true, integration });
...
async function resolveThaneCliPersonForPreference(env: Env, input: {
  workspaceId: string;
  member: NonNullable<Awaited<ReturnType<typeof requireThaneCliWorkspaceMember>>>;
}) {
  const repo = new D1TaskRepository(env.DB);
  const organizationId = coreOrganizationIdForThaneCliWorkspace(input.workspaceId);
  const person = await repo.resolveOrCreatePersonForIdentity({
    organizationId,
    provider: "thane_cli",
    externalWorkspaceId: input.workspaceId,
    externalUserId: input.member.email,
    email: input.member.email,
    displayName: input.member.display_name ?? input.member.handle,
    confidence: 0.85,
    isVerified: true,
    nowIso: nowIso()
  });
  return { repo, organizationId, person };
}

function renderNotificationPreference(input: {
  accountId: string;
  ...
  return {
    ...
    preferredPingLocation: input.preferredPingLocation,
    updatedAt: input.updatedAt,
    ...(input.updatedBy ? { updatedBy: input.updatedBy } : {})
  };
}

async function readThaneCliNotificationPreferenceForSync(env: Env, input: {
  accountId: string;
  workspaceId: string;
  email: string;
}) {
  ...
  const bound = env.DB
    .prepare(
      `SELECT pref.preferred_ping_location, pref.updated_at
       FROM identity_accounts identity
       JOIN person_notification_preferences pref
         ON pref.organization_id = identity.organization_id
        AND pref.person_id = identity.person_id
       WHERE identity.organization_id = ?
         AND identity.provider = 'thane_cli'
         AND identity.external_workspace_id = ?
         AND identity.external_user_id = ?
       LIMIT 1`
    )
    .bind(organizationId, input.workspaceId, input.email);
  const row =
    typeof bound.first === "function"
      ? await bound.first<{ preferred_ping_location?: string | null; updated_at?: string | null }>()
      : null;
  return renderNotificationPreference({
    accountId: input.accountId,
    preferredPingLocation: parsePingLocation(row?.preferred_ping_location) ?? "origin",
    updatedAt: row?.updated_at ?? nowIso()
  });
}

async function handleThaneCliNotifyLocation(request: Request, env: Env): Promise<Response> {
  const email = await requireAuthEmail(request, env);
  if (!email) {
    return Response.json({ ok: false, error: "unauthorized" }, { status: 401 });
  }
  const url = new URL(request.url);
  const payload = request.method === "POST" ? await parseJsonObject<ThaneCliNotifyLocationPayload>(request) : null;
  const workspaceId =
    (request.method === "POST" && typeof payload?.workspaceId === "string" ? payload.workspaceId.trim() : "") ||
  ...
  const { repo, organizationId, person } = await resolveThaneCliPersonForPreference(env, { workspaceId, member });
  const accountId = await accountIdForEmail(email);

  if (request.method === "POST") {
    const preferredPingLocation = parsePingLocation(payload?.preferredPingLocation ?? payload?.location);
    if (!preferredPingLocation) {
      return Response.json({ ok: false, error: "invalid_ping_location" }, { status: 400 });
    }
    const now = nowIso();
    const existing = await repo.getPersonNotificationPreference({
      organizationId,
      personId: person.id
    });
    await repo.upsertPersonNotificationPreference({
      id: existing?.id ?? makeId("pnp"),
      organizationId,
      personId: person.id,
      preferredPingLocation,
      updatedByPlatform: "thane_cli",
      updatedByExternalUserId: email,
      createdAt: existing?.createdAt ?? now,
      updatedAt: now
    });
    return Response.json({
      ok: true,
      preference: renderNotificationPreference({
        accountId,
        preferredPingLocation,
        updatedAt: now,
        updatedBy: "user"
  ...
  const preference = await repo.getPersonNotificationPreference({
    organizationId,
    personId: person.id
  });
  return Response.json({
    ...
    preference: renderNotificationPreference({
      accountId,
      preferredPingLocation: preference?.preferredPingLocation ?? "origin",
      updatedAt: preference?.updatedAt ?? nowIso()
    })
  });
}
...
async function handleThaneCliRequest(request: Request, env: Env): Promise<Response | null> {
  ...
  if (url.pathname === "/v1/thane-cli/webhooks/messages" && request.method === "POST") {
    return handleThaneCliWebhookMessageCreate(request, env);
  }
  if (url.pathname === "/v1/thane-cli/webhooks/messages" && request.method === "GET") {
    return handleThaneCliWebhookMessageHistory(request, env);
  }
  if (url.pathname === "/v1/thane-cli/webhooks/reactions" && request.method === "POST") {
    return handleThaneCliWebhookReactionCreate(request, env);
  }
  ...
  if (url.pathname === "/v1/thane-cli/notify/location" && (request.method === "GET" || request.method === "POST")) {
    return handleThaneCliNotifyLocation(request, env);
  }
  if (url.pathname === "/v1/thane-cli/webhooks" && request.method === "GET") {
    return handleThaneCliWebhookList(request, env);
  }
  if (url.pathname === "/v1/thane-cli/webhooks" && request.method === "POST") {
    return handleThaneCliWebhookCreate(request, env);
  }
  ...
  if (url.pathname === "/v1/thane-cli/read-states" && request.method === "POST") {
    return handleThaneCliReadStateMark(request, env);
  }
